...
class AdmSistem extends MY_Controller
{
    ...
    public function save_master_user()
    {
        ...
        $original = $this->input->post('original_username'); // for edits
        ...
        if ($original) {
            // Update existing (use original username as key)
            $ok = $this->Master_user_model->update($original, $data);
            if ($ok) {
                $this->session->set_flashdata('success', 'Master user updated.');
            } else {
                $this->session->set_flashdata('error', 'Failed to update master user.');
            }
        } else {
            // Create new
            ...
            if ($this->Master_user_model->get($data['username'])) {
                ...
                if (empty($data['password'])) {
                    $this->session->set_flashdata('error', 'Password is required for new users.');
                    redirect('admsistem');
                    return;
                }
                ...
                if ($ok) {
                    $this->session->set_flashdata('success', 'Master user created.');
                } else {
                    $this->session->set_flashdata('error', 'Failed to create master user.');
                }
            }
        }
